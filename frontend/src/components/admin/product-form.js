...
export function Addproduct({ admin, adminToken }) {
  const [airfreshner, setAirfreshner] = useState(false);
  const [otherbenefits, setOtherbenefits] = useState("");
  const [animalFriendly, setAnimalfriendly] = useState(false);
  const [poisonous, setPoisonous] = useState(false);
  const [deathOnConsumption, setDeathOnConsumption] = useState(false);
